...
def build_frequency_map(orders: List[Dict]) -> "collections.Counter":
...
def classify_and_sort(orders: List[Dict], freq_map: "collections.Counter") -> List[Dict]:
    """Помечает каждый заказ группой A/B/C и max_freq. Возвращает отсортированный список."""
    for o in orders:
        unique_offers = set()
        total_qty = 0
        max_freq = 0
        ...
        if num_positions == 1 and total_qty == 1:
            group = "A"
        elif num_positions == 1 and total_qty > 1:
            group = "B"
        else:
            group = "C"
