...
const STATUS: Record<string, { label: string; color: string; border: string }> = {
  ...
  pdf_ready: { label: "Готов",     color: "text-gray-700 bg-gray-100",      border: "border-l-gray-400" },
  done:      { label: "Готово",    color: "text-green-800 bg-green-100",    border: "border-l-green-500" },
      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="shrink-0 text-center w-16">
            {job.orders_found > 0 ? (
              <>
                <div className="text-3xl font-bold text-gray-900 leading-none">{job.orders_found}</div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {job.pdf_files.map((f) => (
                <div key={f} className="flex items-center gap-0 rounded-xl border-2 border-gray-200 overflow-hidden">
                  <button
                    onClick={() => handlePrint(f)}
                    disabled={printingFile === f}
                    className="flex items-center gap-2 px-3.5 py-2 text-xs font-semibold text-white bg-gray-900 hover:bg-gray-800 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-150"
                  >
                    {printingFile === f ? (
                  ...
                  <a
                    ...
                    rel="noreferrer"
                    title="Открыть PDF"
                    className="px-3 py-2 text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition-colors border-l-2 border-gray-200"
                  >
              ...
              {canRetry && (
                <button
                  onClick={() => onRetry(job)}
                  disabled={retrying}
                  className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-xl border-2 border-gray-200 text-gray-600 hover:border-gray-400 hover:text-gray-900 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-150"
                >
                  {retrying ? (
